...
Run the `train_search.py` script to search for the optimal adaptive meta-graph. The script automatically identifies the best combination of meta-paths to improve drug–target interaction prediction performance.

Run the `train.py` script to perform drug–target interaction (DTI) prediction based on the selected adaptive meta-graph. This script applies the optimal meta-graph obtained from the previous search stage to generate 
