...
def generate_data(seed, num_drug, data_path):
    ...
    for fold, (train_indices, test_indices) in enumerate(kf.split(neg_pos)):
        ...
        arrtrain = np.asarray(neg_train_fold, dtype=object)
        arrval = np.asarray(neg_val_fold, dtype=object)
        arrtest = np.asarray(neg_test_fold, dtype = object)
    np.savez("./preprocessed/neg_pairs_offset", train=arrtrain, val=arrval, test=arrtest, allow_pickle=False)
